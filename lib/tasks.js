...
const PEOPLE = [
  ...
  'Shajee', 'Saji', 'Jason', 'Grant', 'Farenaaz',
  'Gamma Waves', 'Copywriter', 'Web Developer', 'Finance',
];
...
function buildToday(tasks) {
  ...
  const waiting = rest.filter(t =>
    !now.includes(t) && (t.status === 'Blocked' || t.status === 'Waiting' || t.waitingOn)
  );
  const backlog = rest.filter(t => !now.includes(t) && !waiting.includes(t));
...
module.exports = {
  ...
  normaliseTask, buildToday, waitingBy, isDone, PRIORITY_RANK,
};
