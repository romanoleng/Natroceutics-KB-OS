...
module.exports = {
  ...
  async insights() {
    ...
    const soonUnits = soon.reduce((s, r) => s + (n(r['Total QTY']) || 0), 0);
    const out = [];

    out.push(insight({
      id: 'wh-expiry',
      headline: soon.length ? 'Stock approaching its best-before date' : 'No stock is close to expiry',
      value: `${soonUnits.toLocaleString('en-GB')} units`,
      comparison: `of ${units.toLocaleString('en-GB')} on hand, expiring within ${HORIZON} months`,
      why: soon.length
        ? `Soonest is ${soon.map(r => r['Earliest BBD']).sort()[0]}. Stock that expires before it sells is money already spent.`
        : `Earliest best-before across the warehouse is ${dated.map(r => r['Earliest BBD']).sort()[0] || 'unknown'}.`,
      tone: soon.length ? 'down' : 'up',
    }));

    out.push(insight({
      id: 'wh-traceable',
      headline: 'Stock traceable to a batch',
      value: `${dated.length} of ${rows.length} SKUs`,
      comparison: `carrying a best-before date, from the ${rows.length}-SKU warehouse sheet`,
      why: 'Units with no batch have no expiry, so they are invisible to every check above.',
      tone: dated.length === rows.length ? 'up' : 'flat',
    }));
    return out;
  },
  ...
  async actions() {
    ...
    if (soon.length) {
      out.push(action({
        id: 'wh-decide-expiry',
        title: `${soon.length} SKUs expiring within ${HORIZON} months`,
        next: `Decide the route for ${soon.slice(0, 3).map(x => x.r.SKU).join(', ')}${soon.length > 3 ? ' and the rest' : ''}: promote, bundle, or write off. Doing nothing chooses write-off by default.`,
        why: `${soon.reduce((s, x) => s + (n(x.r['Total QTY']) || 0), 0).toLocaleString('en-GB')} units, soonest ${soon[0].r['Earliest BBD']}.`,
        severity: soon[0].m <= 3 ? 'high' : 'medium',
      }));
    }
    ...
    if (unbatched.length) {
      ...
      out.push(action({
        id: 'wh-unbatched',
        title: `${u} units carry no batch`,
        next: `Ask the warehouse for the batch and best-before on ${unbatched.map(r => r.SKU).join(', ')}, then re-upload the SOH workbook.`,
        why: 'Those units cannot be expiry-checked or traced in a recall.',
        severity: 'low',
      }));
    }
    return out;
