...
function normaliseRecord(r) {
  ...
  // Sync-driven tables (Shopify, Amazon, warehouse feeds etc.) use a "Last Synced"-style
  // field instead of "Last Modified" — checked next so those tables get a real update
  // date rather than falling back to createdTime. Covers "Stock Last Synced", "Last Synced",
  // "Last Sync", "Synced At" — add new variants here rather than in individual pages.
  const syncField = clean['Stock Last Synced'] || clean['Last Synced'] || clean['Last Sync'] || clean['Synced At'] || null;
  normalised._updatedAt = clean['Last Note At'] || clean['Last Modified'] || clean['Last modified'] || syncField || null;
  normalised._syncSourced = !!(!clean['Last Note At'] && !clean['Last Modified'] && !clean['Last modified'] && syncField);
  return normalised;
}
