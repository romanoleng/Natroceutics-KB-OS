...
/*
 * Once one request 429s, every other Airtable call in the next minute will
 * too (the monthly cap is account-wide) — so remember it and fail instantly
 * instead of letting every unmirrored table spend ~3s of retries per page
 * load. A page with a dozen unmirrored tables goes from "sluggish" to
 * DB-only speed; a healthy account clears the flag after 60s.
 */
let quotaBlockedUntil = 0;

async function withRetry(label, fn) {
  if (Date.now() < quotaBlockedUntil) {
    const err = new Error('Airtable rate limited (cached) — skipping call');
    err.statusCode = 429;
    throw err;
  }
    ...
    try {
      ...
      if (!isRateLimited(err) || attempt >= RETRY_DELAYS_MS.length) {
        if (isRateLimited(err)) {
          ...
          console.error(`[airtable] rate limited/quota exceeded after ${attempt + 1} attempts: ${label} — backing off 60s`);
        }
        throw err;
      }
...
export async function getMECostModel()     { return fetchFromRegion('AIRTABLE_ME_BASE_ID', ME_TABLES.COST_MODEL); }
