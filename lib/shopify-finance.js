...
async function getShopifyFinance(months) {
  ...
  return {
    ...
    //
    // recordId and the table coordinates are carried through because these
    // rows are editable in the UI, and a write needs to say which row it means.
    model: model.map(r => ({
      ...f(r),
      id: r.recordId || r.id || f(r).Key,
      _baseId: r._baseId || baseId(),
      _tableId: r._tableId || UK.tables.COST_MODEL,
    })),
...
/** Klaviyo, measured. Empty arrays until scripts/klaviyo-pull.js has run. */
async function getKlaviyo() {
  const [lists, flows, campaigns, revenue] = await Promise.all([
    read('KLAVIYO_LISTS'), read('KLAVIYO_FLOWS'),
    read('KLAVIYO_CAMPAIGNS'), read('KLAVIYO_REVENUE'),
  ]);
  return {
    lists: lists.map(f), flows: flows.map(f),
    campaigns: campaigns.map(f), revenue: revenue.map(f),
    connected: flows.length > 0 || lists.length > 0,
  };
}
module.exports.getKlaviyo = getKlaviyo;

/** Mailchimp for a region (SA today). Empty until mailchimp-pull.js has run. */
async function getMailchimp(regionKey = 'SA') {
  const b = BASES[regionKey];
  const bid = resolveBaseId(b.envVar);
  const rd = async k => (b.tables[k] ? (await fetchFromMirror(bid, b.tables[k])) || [] : []);
  const [audiences, campaigns, automations] = await Promise.all([
    rd('MAILCHIMP_AUDIENCES'), rd('MAILCHIMP_CAMPAIGNS'), rd('MAILCHIMP_AUTOMATIONS'),
  ]);
  return {
    audiences: audiences.map(f), campaigns: campaigns.map(f), automations: automations.map(f),
    connected: audiences.length > 0 || campaigns.length > 0,
  };
}
module.exports.getMailchimp = getMailchimp;

/** Payouts, year-to-date and the live affiliate roster. */
async function getFinanceDepth() {
  const [payouts, ytd, affiliates, affMonthly] = await Promise.all([
    read('SHOPIFY_PAYOUTS'), read('SHOPIFY_YTD'),
    read('AFFILIATES_LIVE'), read('AFF_MONTHLY'),
  ]);
  return {
    payouts: payouts.map(f),
    ytd: (ytd.map(f))[0] || null,
    affiliates: affiliates.map(f),
    affMonthly: affMonthly.map(f),
  };
}
module.exports.getFinanceDepth = getFinanceDepth;
