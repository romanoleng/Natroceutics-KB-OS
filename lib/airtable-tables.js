...
const TABLES = {
  ...
  /**
   * Cross-base findings. OS-native: there is no Airtable counterpart and there
   * must not be one — a full sync would delete every finding.
   *
   * A finding is TWO RECORDS THAT DISAGREE, never a general observation. It
   * lives here rather than per-base because the useful ones span bases: a
   * figure quoted in ME.FINANCE that fills a gap UK.COST_MODEL calls PENDING
   * is invisible from either page alone, which is the whole reason for a
   * single feed rather than a card bolted onto each page.
   *
   * Findings are ADVISORY. This table is never read back into a P&L and
   * nothing here mutates business data.
   */
...
const SA_TABLES = {
  ...
  // SA runs Mailchimp where UK runs Klaviyo. OS-native: there is no Airtable
  // counterpart, and we own this schema.
  MAILCHIMP_AUDIENCES:  'os:sa-mailchimp-audiences',
  MAILCHIMP_CAMPAIGNS:  'os:sa-mailchimp-campaigns',
  MAILCHIMP_AUTOMATIONS:'os:sa-mailchimp-automations',
};
...
const UK_TABLES = {
  ...
  AFFILIATES_LIVE:      'os:uk-affiliates-live',
  SHOPIFY_PAYOUTS:      'os:uk-shopify-payouts',
  SHOPIFY_YTD:          'os:uk-shopify-ytd',
  // Amazon Transparency enrolment, one row per ASIN, uploaded from the
  // Transparency export in Seller Central. Its own table rather than columns on
  // UK.AMAZON: that one is REPLACED by the Sellerboard stock import every night
  // and would erase the enrolment status by morning.
  TRANSPARENCY:         'os:uk-transparency',
  // Passive time tracking. Replaces Clockify, which Romano abandoned because
  // he always forgot to press start — so this has no start button.
  TIME_SESSIONS:        'os:time-sessions',
...
const ME_TABLES = {
  ...
  /**
   * What it costs to RUN the ME store, as opposed to what the store earns.
   * OS-native, and separate from FINANCE on purpose: the Gamma Waves quote
   * lived in FINANCE until 3 Aug 2026 and shared none of its fields, so ten
   * fully populated rows rendered as ten rows of dashes through the revenue
   * table. Finance is bills and revenue; the cost model is the structure
   * behind them. UK keeps the same split under `os:uk-cost-model`.
   */
  COST_MODEL:    'os:me-cost-model',
...
/* ── AFFILIATE RECRUITMENT CRM (appjw0F7rb4UFIge1) ───────── */
const ACRM_TABLES = {
  PROSPECTS:    'tblQnGUAhHRJX1cCU',
  OUTREACH_LOG: 'tblZ7JXKjk6CHWpy8',
};

/* ── GCC AFFILIATE EXPANSION (appagZzceGkeis9uy) ──────────── */
const GCC_TABLES = {
  INFLUENCERS:           'tblj9zqF0GmkHQ0bE',
  PARTNERS_DISTRIBUTORS: 'tblfadi4y0HVPb7VC',
};

/* ── GLOBAL STOCK (appZYE1iJ6tvVuPiA) ─────────────────────── */
const GSTOCK_TABLES = {
  BULGARIA_STOCK: 'tblDq6gGQXccm8xTN',
};

/* ── CUSTOMER DB (appQyPV2dmVqE9CED) ──────────────────────── */
const CUSTDB_TABLES = {
  AFFILIATES_ME_GCC: 'tblHpLLk28hs6NsO7',
};

/* ── CUSTOMER MASTER (appY6dy3AGJBz9Uym) ──────────────────── */
const CUSTMASTER_TABLES = {
  CONTACTS: 'tblzTAZxPtjXGrg82',
};
...
const BASES = {
  ...
  // Added 1 Aug 2026 after discover-bases.js found five live Natroceutics
  // bases outside the registry. Small but real: two of them are customer data.
  ACRM:   { key: 'ACRM',   label: 'Affiliate Recruitment', envVar: 'AIRTABLE_ACRM_BASE_ID', defaultBaseId: 'appjw0F7rb4UFIge1', tables: ACRM_TABLES },
  GCC:    { key: 'GCC',    label: 'GCC Expansion',   envVar: 'AIRTABLE_GCC_BASE_ID',       defaultBaseId: 'appagZzceGkeis9uy', tables: GCC_TABLES },
  GSTOCK: { key: 'GSTOCK', label: 'Global Stock',    envVar: 'AIRTABLE_GSTOCK_BASE_ID',    defaultBaseId: 'appZYE1iJ6tvVuPiA', tables: GSTOCK_TABLES },
  CUSTDB: { key: 'CUSTDB', label: 'Customer DB',     envVar: 'AIRTABLE_CUSTDB_BASE_ID',    defaultBaseId: 'appQyPV2dmVqE9CED', tables: CUSTDB_TABLES },
  CUSTM:  { key: 'CUSTM',  label: 'Customer Master', envVar: 'AIRTABLE_CUSTM_BASE_ID',     defaultBaseId: 'appY6dy3AGJBz9Uym', tables: CUSTMASTER_TABLES },
};
