...
async function getOrderMetric(since, until) {
  ...
  // Klaviyo returns bucket boundaries in UTC while we request Europe/London.
  // Under BST that pushes each month boundary back an hour, so a bucket for
  // April arrives labelled "2026-03-31". Slicing the first seven characters
  // therefore shifted every month from April onward by one and made July look
  // like it had 3 orders instead of 68 — which I misread as the Shopify
  // integration having died. Nudge forward half a day before taking the month.
  const monthOf = d => {
    const t = new Date(d);
