...
const FEEDS = [
  ...
  {
    // The OS checking ITSELF, not a source arriving. Watched here because two
    // surfaces now read it — /status and Mission Control — and a pass that
    // silently stopped would leave both showing yesterday's findings as
    // today's, which is the failure this whole page exists to catch.
    key: 'findings', label: 'Findings pass', region: 'All',
    sources: ['findings-pass'], everyHours: 24,
    detail: 'Cross-checks the OS against itself and writes what disagrees',
  },
  {
