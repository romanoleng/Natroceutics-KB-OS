...
/**
 * "6/05/2026" or "6/05/2026 9:26:04 PM" (D/MM/YYYY) → "2026-05-06", and
 * "2026-05-06" straight through.
 *
 * Both forms genuinely reach the builders. coerce() in scripts/lib/csv.js
 * normalises any bare D/MM/YYYY cell to ISO before a builder ever sees it, so
 * the dashboard exports arrive already converted; the orders export carries a
 * time component ("6/05/2026 9:26:04 PM"), which coerce's anchored regex does
 * not match, so that one still arrives raw.
 *
 * Accepting only the slash form silently dropped every row of the two
 * dashboard exports — the builders `continue` on a null date, so the import
 * reported "recognised the file but found no usable rows" with nothing to say
...
/* ── fee / ad columns ──────────────────────────────────────
 * Sellerboard ships two shapes of the same dashboard export. The site export
 * breaks fees out into components and names COGS "ProductCost Sales"; the
 * emailed DashboardTotals export carries a single aggregate "AmazonFees" and
 * calls COGS "Cost of Goods". Reading only one shape does not fail loudly — it
 * sums nothing and writes 0, so a day with £200 of sales imports with no fees
 * and no COGS and looks far more profitable than it was. Read both.
 */
const FEE_COLUMNS = [
...
/** First column present and non-blank, else null — absent ≠ zero. */
function firstPresent(r, ...keys) {
  for (const k of keys) if (r[k] !== undefined && r[k] !== '') return r[k];
  return null;
}

/** Component fees where the export breaks them out, the aggregate where it does not. */
function feeTotal(r) {
  const hasComponents = FEE_COLUMNS.some(c => r[c] !== undefined);
  if (hasComponents) return FEE_COLUMNS.reduce((s, c) => s + absN(r[c]), 0);
  return absN(firstPresent(r, 'AmazonFees'));
}

const cogsTotal = r => absN(firstPresent(r, 'ProductCost Sales', 'Cost of Goods'));
  ...
  for (const r of rows) {
    ...
    out.push({
      ...
      fields: {
        ...
        'Amazon Fees £': round2(feeTotal(r)),
        'Ad Spend £': round2(AD_COLUMNS.reduce((s, c) => s + absN(r[c]), 0)),
        'COGS £': round2(cogsTotal(r)),
...
function parseSellerboardFile(text, ctx = {}) {
  ...
  return {
    type,
    records: type.build(objects, context),
    rowCount: objects.length,
    // The first row as the builders actually see it, for the "recognised but
    // built nothing" error. Cells are post-coerce(), which is the whole point:
    // that step is where a date stops looking like the source file.
