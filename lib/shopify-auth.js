...
async function getAccessToken() {
  ...
  // Trimmed: these get pasted into a CLI prompt, and a trailing newline turns a
  // correct client ID into one Shopify has never heard of. The resulting error
  // is "application_cannot_be_found", which reads like the wrong store.
  const id = (realEnv('SHOPIFY_CLIENT_ID') || '').trim();
  const secret = (realEnv('SHOPIFY_CLIENT_SECRET') || '').trim();
  ...
  if (!res.ok) {
    ...
    // A fingerprint, not the value: enough to compare against .env.local by eye
    // and settle whether the two environments hold the same string.
    const fp = `id ${id.length} chars ending "${id.slice(-4)}", ` +
               `secret ${secret.length} chars starting "${secret.slice(0, 5)}"`;
    const why = /application_cannot_be_found/.test(body)
      ? `no app with this SHOPIFY_CLIENT_ID is installed on ${host} (${fp})`
      : /invalid_client/.test(body)
        ? `SHOPIFY_CLIENT_SECRET does not match this client ID (${fp})`
        : /invalid_request/.test(body)
          // Shopify returns this for a malformed credential pair, which in
          // practice means one of the two is truncated or from another app.
          ? `Shopify rejected the credential pair as malformed (${fp}). ` +
            'A correct pair is a 32-character id and a secret starting "shpss_".'
