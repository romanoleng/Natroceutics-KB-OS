...
const RECORD_TYPES = {
  ...
  REGISTRATIONS: {
    ...
    area: ['Market', 'Country'],
    lanes: [
      ...WORK_LANES.slice(0, 3),
      { key: 'done', title: 'Registered', hint: 'approved or complete', tone: 'backlog',
        match: s => ['Registered', 'Approved', 'Done', 'Complete', 'Completed'].includes(s) },
    ],
...
function buildRecordLanes(records, typeKey) {
  ...
  // Anything matching NO lane still gets shown. A record with an unexpected
  // status would otherwise disappear from the page entirely while remaining in
  // the database and in the counts — a silent drop, and the same failure class
  // as an error dressed as an empty state. The lane names the statuses it
  // caught so the fix is obvious rather than mysterious.
  const claimed = new Set(lanes.flatMap(l => l.tasks.map(t => t.id)));
  const orphans = records.filter(r => !claimed.has(r.id)).sort(recordSort);
  if (orphans.length) {
    const statuses = [...new Set(orphans.map(o => o.status || '(blank)'))];
    lanes.push({
      key: 'unclassified',
      title: 'Unclassified',
      hint: `status not recognised: ${statuses.join(', ')}`,
      tone: 'waiting',
      tasks: orphans,
    });
  }
